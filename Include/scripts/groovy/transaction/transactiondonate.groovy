...
class transactiondonate {
	...
	@When('I fill in the donation "(.*)" number')
	def I_fill_in_the_system_donation_number(String number) {
		WebUI.callTestCase(findTestCase('Web UI Tests/Advance Tests/Pages/Donation/I fill in the donation transaction'), [('number') : number], FailureHandling.STOP_ON_FAILURE)
	}


	@Then('I validate the transaction')
	def I_validate_the_transaction() {
		WebUI.callTestCase(findTestCase('Web UI Tests/Advance Tests/Pages/Validation/validate'), [:],
		FailureHandling.STOP_ON_FAILURE)
	}
}